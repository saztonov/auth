...
- **плитки доступных порталов** (список — `src/config/portals.ts`; фильтруются по client-роли/группе
  из токена — см. «Модель доступа к порталам» ниже);
...
как BillHub, или client-роль `access` — как EstiMat) — это остаётся источником истины, витрина его не
подменяет. Дополнительно (с 2026-07-06) витрина **фильтрует плитки** по тому же признаку, чтобы не
показывать порталы, к которым у пользователя заведомо нет доступа:
- клиенту `su10-launcher` в `su10-realm.yaml` добавлены protocolMappers `resource_access.estimat.roles`
  (client-роль EstiMat) и `groups` (членство, гейт BillHub — `billhub-active`), оба с `id.token.claim`
  (эта SPA не делает `loadUserInfo` — `user.profile` только из id_token, в отличие от мапперов на
  клиентах `estimat`/`billhub`, рассчитанных на их BFF-бэкенды);
- `portals.ts`: `requiredRole` (client-роль, EstiMat) или `requiredGroup` (группа, BillHub) —
  `hasPortalAccess()` fail-closed (нет нужной роли/группы в токене → плитка скрыта).

⚠️ Порядок раскатки важен: KC-маппер накатывать (`config-cli`) ДО пересборки/редеплоя витрины — иначе
в окне между ними у витрины ещё нет данных в токене, а логика уже fail-closed → временно 0 плиток у
всех. Также: уже открытые сессии в `sessionStorage` не подхватят новые claim'ы без повторного логина.
