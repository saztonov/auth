...
### Гейты до флипа
- ✅ **Деплой витрины** в `/opt/infra/launcher/dist/` — сделано (2026-07-06).
- ✅ **bcrypt-verify + перехэш на БОЕВЫХ данных — подтверждено** (2026-07-06, `verify-rehash.sh`): вход
  реальным СТАРЫМ паролем (не сброс через admin-консоль) на 3 аккаунтах (`esk@test.com`,
  `eugsu10@gmail.com`, `test1@test.com`) через `auth.su10.ru` → все три `algorithm=argon2`. Это закрывает
  главный технический риск миграции — SPI реально работает на боевых импортированных хэшах, не только
  на синтетике `bcrypt-poc`. ⚠️ Проверяет только механизм Keycloak (bcrypt+rehash), **не** собственный
  флоу BillHub (резолв/роль/гейт с его стороны) — вход был напрямую на `auth.su10.ru`, не через `rp.su10.ru`.
- ⬜ **CSS-фикс темы** (billhub) + рестарт KC (кэш темы в проде) — некритично для флипа, но по вашему
  плану «сначала тема».
- ⬜ **Канарейка BillHub-стороны** (единственный оставшийся критический гейт): `AUTH_MODE=keycloak` на
  ОДНОМ инстансе, реальный вход через `rp.su10.ru` (не напрямую на auth.su10.ru) → проверить: резолв
  `billhub_user_id` → внутренний id, роль из БД BillHub, гейт по `billhub-active`/`pending` именно на
  стороне BillHub-бэкенда, SSO, logout. Можно переиспользовать те же 3 аккаунта (пароль уже bcrypt→argon2,
  на проверку логики BillHub это не влияет).
- ⬜ **Полный флип** `AUTH_MODE=keycloak` — только после зелёной канарейки BillHub-стороны. Откат =
  `AUTH_MODE=standalone` (не удалять `password_hash`/standalone-код/refresh-таблицы до конца окна отката).
