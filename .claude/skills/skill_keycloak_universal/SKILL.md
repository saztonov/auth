...
- **Мапперы:** `audience` (`aud=<portal>`), `email`, `preferred_username`. Для гейта-по-группе — **Group Membership** (`claim.name: groups`, в access token). При своей БД — user-attribute mapper `<portal>_user_id` (`oidc-usermodel-attribute-mapper`) в access+id token. ⚠️ **KC 26 по умолчанию отбрасывает неописанные (unmanaged) атрибуты** — чтобы `<portal>_user_id` реально сохранялся при create/import через Admin API, в realm-as-code задать `userProfile` со `unmanagedAttributePolicy: ADMIN_EDIT` (обязательно объявив стандартные username/email/firstName/lastName, иначе политика игнорируется) или объявить атрибут явно; иначе маппер пробросит пустоту. Пример — секция `userProfile` в `su10-realm.yaml`. **Client-роли для авторизации в токен НЕ тянуть** (роли держит портал у себя), кроме случая гейта-по-роли (тогда `usermodel-client-role-mapper` с `resource_access.<portal>.roles`).
...
- CLI: режимы `preflight|import|verify|reconcile|report`; dry-run, checkpoint/resume, рейт-лимит, без логирования секретов/хэшей. Backup БД + realm-export до старта; сохранить старый механизм входа на окно отката.
